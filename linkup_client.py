...
def linkup_search(query: str, depth: str = "standard", output_type: str = "searchResults", max_results: int = 5, **kwargs):
    """Module-level convenience wrapper so other modules can call linkup_search() directly."""
    api_key = os.getenv("LINKUP_API_KEY")
    if not api_key:
        raise ValueError("LINKUP_API_KEY not found in .env")
    client = LinkupClient(api_key=api_key)
    return client.search(query=query, depth=depth, output_type=output_type, include_images=False, **kwargs)
...
Return all qualifying job links and details. Prioritize official {company_name} career pages over third-party job boards."""
    ...
    def get_company_profile(self, company: str, **kwargs) -> dict:
...
Focus on authoritative sources: official blog, press releases, SEC filings, TechCrunch, Bloomberg."""
        ...
        print(f"      🔍 Searching company overview, news, financials...")
        response = self.client.search(
            query=query,
            depth="standard",
            output_type="searchResults",
    ...
    def get_company_sentiment(self, company: str, **kwargs) -> dict:
...
Summarize: overall sentiment score (1-10), top 3 pros, top 3 cons, red flags if any."""
        ...
        print(f"      🔍 Searching reviews, culture, compensation...")
        response = self.client.search(
            query=query,
            depth="standard",
            output_type="searchResults",
