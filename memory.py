...
class Memory:
    """
    Backwards-compatible OO wrapper around the module-level memory functions.

    Some parts of the codebase (e.g., app.py / AgentCore wiring) expect a `Memory`
    object. The underlying implementation in this repo is function-based + SQLite.
    This wrapper preserves the newer implementation while keeping old imports working.
    """
    ...
    # Writes
    def store_turn(self, session_id: str, role: str, text: str, **kwargs):
        return store_turn(session_id=session_id, role=role, text=text, db_path=self.db_path, **kwargs)

    def store_artifact(self, session_id: str, type: str, content, **kwargs):
        return store_artifact(session_id=session_id, type=type, content=content, db_path=self.db_path, **kwargs)

    def store_fact(self, session_id: str, fact_type: str, key: str, value: str, **kwargs):
        return store_fact(session_id=session_id, fact_type=fact_type, key=key, value=value, db_path=self.db_path, **kwargs)

    # Reads
    def get_turns_by_session(self, session_id: str, limit: int = 100):
        return get_turns_by_session(session_id=session_id, limit=limit, db_path=self.db_path)

    def get_artifacts_by_session(self, session_id: str, limit: int = 100):
        return get_artifacts_by_session(session_id=session_id, limit=limit, db_path=self.db_path)

    def get_facts_by_session(self, session_id: str, limit: int = 100):
        return get_facts_by_session(session_id=session_id, limit=limit, db_path=self.db_path)

    def search_facts_by_key(self, session_id: str, key: str, limit: int = 50):
        return search_facts_by_key(session_id=session_id, key=key, limit=limit, db_path=self.db_path)

    def get_context(self, session_id: str, **kwargs):
        return get_context(session_id=session_id, db_path=self.db_path, **kwargs)

    # Users/sessions
    def register_user(self, user_id: str):
        return register_user(user_id=user_id, db_path=self.db_path)
